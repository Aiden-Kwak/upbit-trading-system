...
│   ├── signal_engine.py     # VB + MB + MR + VS 4전략 앙상블
...
5. **signal_engine 평가** — 4개 전략 계산 → 등급 A/B만 후보
...
### 3. 4가지 전략 (signal_engine)

각 코인에 대해 4개 전략을 독립적으로 평가. 등급 A/B만 매수 후보가 됩니다.
...
**② 본전 방어 (Breakeven)**
한 번이라도 `+2%` 이상 갔으면 `breakeven_armed`. 이후 PnL이 `+0.2%` 아래로 떨어지면 `SELL_BREAKEVEN`으로 이탈. 이익 물리는 것을 방지.
...
| **live** | O | 실계좌 | 실거래 |
| **dry** | X | 실계좌 조회 | 로직 검증 (주문만 스킵) |
| **paper** | X | 가상잔고 100만원 | end-to-end 시뮬레이션 (현재 가동 중) |
